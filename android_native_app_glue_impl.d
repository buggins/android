...
static void print_cur_config(android_app* android_app) {
...
static void android_app_write_cmd(android_app* android_app, byte cmd) {
    if (write(android_app.msgwrite, &cmd, cmd.sizeof) != cmd.sizeof) {
        LOGE("Failure writing android_app cmd: %s\n", strerror(errno));
    }
}

static void android_app_set_input(android_app* android_app, AInputQueue* inputQueue) {
    pthread_mutex_lock(&android_app.mutex);
...
static void android_app_set_window(android_app* android_app, ANativeWindow* window) {
    pthread_mutex_lock(&android_app.mutex);
...
static void android_app_set_activity_state(android_app* android_app, byte cmd) {
    pthread_mutex_lock(&android_app.mutex);
...
static void android_app_free(android_app* android_app) {
    pthread_mutex_lock(&android_app.mutex);
